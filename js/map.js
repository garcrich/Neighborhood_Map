...
function initMap() {
    ...
    //map markers
    banks.allMarkers = [];
    var bank_Markers = banks.allMarkers;

    function makeMarkers() {
        for (var i = 0; i < banks().length; i++) {
            marker = new google.maps.Marker({
                position: {
                    lat: banks()[i].latitude,
                    lng: banks()[i].longitude
                },
                map: map,
                title: banks()[i].title,
            });
            bank_Markers.push(marker); //push markers because Google Maps does not support this feature

            google.maps.event.addListener(bank_Markers[i], "click", (function(bank_Markers, i) {
                return function() {
                    ...
                    infowindow.open(map, bank_Markers[i]);

                    if (bank_Markers[i].getAnimation() !== null) {
                        bank_Markers[i].setAnimation(null);
                        infowindow.close(map, bank_Markers[i]);
                    } else {
                        bank_Markers[i].setAnimation(google.maps.Animation.BOUNCE);
                    }
                    }
            })(bank_Markers, i));
        };
    ...
    for (var i = 0; i < ((banks()[1].koTitle).length); i++) {
